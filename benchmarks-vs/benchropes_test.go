...
import (
	...
	rope "github.com/chewxy/skiprope"
...
func init() {
	rs := []rune(a)
	randoms = make([]int, 0, 40000000)

	for i := 0; i < 40000000; i++ {
		randoms = append(randoms, rnd.Intn(len(rs)-1))
	}
	randWords = strings.Fields(a)
}

func BenchmarkNaiveRandomInsert(b *testing.B) {
	buf := &naivebuffer{bytes.NewBuffer([]byte(a))}
	var randTrack int
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		at := randoms[randTrack]
		word := randWords[rand.Intn(len(randWords))]
		rs := []rune(word)
		buf.Insert(at, word)
...
func BenchmarkERopeRandomInsert(b *testing.B) {
	r := eugenerope{er.L(a)}
	b.ResetTimer()
